...
namespace p1_Motor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            cProyecto cp1 = new cProyecto("Ateca1", "Josep", new DateTime(2020,11,8), dtoutput1, new cMaquina(cMaquina.model.Hidrosolver), cProyecto.estado.EnCurso);
            cProyecto cp2 = new cProyecto("Ateca1", "Josep", dtinput2, dtoutput2, new cMaquina(cMaquina.model.Hidrosolver), cProyecto.estado.EnCurso);
            Console.WriteLine(dtoutput1.ToString("y-M-d H:m:ss"));
            cp1.WriteXML();                             
            //cp1.ReadXML();
            Console.ReadKey();
        }         
